...
BEGIN {push (@INC, "./lib");}
use jmdict;  use jmdictxml ('%EX2ID');  
...
main: {
	...
	if (!getopts ("b:c:e:wxvu:p:r:d:h", \%::Opts)) { usage (1); }
	if ($::Opts{h}) { usage (0); }
	...
	process ($dbh, $infn, $begin, $count);
	msg_summary (\%::Msgs);
	if (!$::Opts{w}) { $dbh->disconnect(); } }
	
sub process { my ($dbh, $infn, $begin, $count) = @_;
	my ($ln, $aln, $bln, $jtxt, $etxt, $kwds, $cntr, $idxlist); 
	open (FIN, "<:utf8", $infn) || die ("Can't open $infn: $!\n");
	while ($aln = <FIN>) {
	    next if ($. < $begin);
	    if ($. == 1 and ((0 >= substr ($aln, 0, 1) eq "\x{FEFF}") or
			     (0 >= substr ($aln, 0, 1) eq "\x{FFFE}"))) {
		$aln = substr ($aln, 1); }
		...
		addindexes ($dbh, $jtxt, $etxt, $kwds, $::Lnnum, $idxlist);
		if (!$::Opts{w} && !$::Opts{x} && !($cntr % 500 == 0)) {
		    $dbh->commit(); } }
	    last if ($cntr >= $count);} 
	if (!$::Opts{w} && !$::Opts{x}) { $dbh->commit(); } }


sub addindexes { my ($dbh, $jtxt, $etxt, $kwds, $lnnum, $idxlist) = @_;
	# $idxlist data: [(ktxt,rtxt,sens,atxt),...]
	my ($xrefs, $entr, $src);
	$xrefs = resolve ($dbh, $idxlist);
	$xrefs = prunedups ($xrefs);
	if (scalar (@$xrefs) == 0) {
	    msg ("No indexable words, line skipped");  return;}
...
Options:
	...
	-w -- Parse only.  Do access database at all: do not write
	    anything to it, nor resolve index words from it.
	-x -- Resolve only.  Access database read-only in order to 
	    resolve index words; do not write anything to it.
