...
our(@VERSION) = (substr('$Revision$',11,-2), \
	...
	my $rfreq = dbread ($dbh, "SELECT x.* $com JOIN rdng r ON r.entr=e.id JOIN rfreq x ON x.entr=r.entr AND x.rdng=r.rdng $where;", $args);
	my $rinf  = dbread ($dbh, "SELECT x.* $com JOIN rdng r ON r.entr=e.id JOIN rinf  x ON x.entr=r.entr AND x.rdng=r.rdng $where;", $args);
	my $audio = dbread ($dbh, "SELECT x.* $com JOIN rdng r ON r.entr=e.id JOIN audio x ON x.entr=r.entr AND x.rdng=r.rdng $where;", $args);
	my $kanj  = dbread ($dbh, "SELECT k.* $com JOIN kanj k ON k.entr=e.id $where ORDER BY k.entr,k.kanj;", $args);
	my $kfreq = dbread ($dbh, "SELECT x.* $com JOIN kanj k ON k.entr=e.id JOIN kfreq x ON x.entr=k.entr AND x.kanj=k.kanj $where;", $args);
	my $kinf  = dbread ($dbh, "SELECT x.* $com JOIN kanj k ON k.entr=e.id JOIN kinf  x ON x.entr=k.entr AND x.kanj=k.kanj $where;", $args);
	my $sens  = dbread ($dbh, "SELECT s.* $com JOIN sens s ON s.entr=e.id $where ORDER BY s.entr,s.sens;", $args);
	my $gloss = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN gloss x ON x.entr=s.entr AND x.sens=s.sens $where ORDER BY x.entr,x.sens,x.gloss;", $args);
	my $misc  = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN misc  x ON x.entr=s.entr AND x.sens=s.sens $where;", $args);
	my $pos   = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN pos   x ON x.entr=s.entr AND x.sens=s.sens $where;", $args);
	my $fld   = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN fld   x ON x.entr=s.entr AND x.sens=s.sens $where;", $args);
	my $restr = dbread ($dbh, "SELECT x.* $com JOIN rdng r ON r.entr=e.id JOIN restr x ON x.entr=r.entr AND x.rdng=r.rdng $where;", $args);
	my $stagr = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN stagr x ON x.entr=s.entr AND x.sens=s.sens $where;", $args);
	my $stagk = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN stagk x ON x.entr=s.entr AND x.sens=s.sens $where;", $args);
	my $xref  = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN xref  x ON x.entr=s.entr  AND x.sens=s.sens  $where;", $args);
	my $xrer  = dbread ($dbh, "SELECT x.* $com JOIN sens s ON s.entr=e.id JOIN xref  x ON x.xentr=s.entr AND x.xsens=s.sens $where;", $args);
