...
def add_restr_summary (entries):
	...
	# that has a restr list.  The ._RESTR attribut valiue is a list 
	...
	#
	# Also add a _RESTR boolean flag to the entry if there are 
	# _restr items on any reading.

	for e in entries:
	    if not hasattr (e, '_rdng') or not hasattr (e, '_kanj'): continue
	    for r in e._rdng:
		rt = restrtxts (r._restr, 'kanj', e._kanj)
	        if rt: r._RESTR = rt
		e._RESTR = 1
...
def add_stag_summary (entries):
	...
	for e in entries:
		...
		if getattr (s, '_stagr', None):
		    rt.extend (restrtxts (s._stagr, 'rdng', e._rdng))
		if getattr (s, '_stagk', None):
		    rt.extend (restrtxts (s._stagk, 'kanj', e._kanj))
		if rt:
		    s._STAG = rt
