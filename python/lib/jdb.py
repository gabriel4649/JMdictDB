...
def setkeys (e, id=0):
        ...
        for n,s in enumerate (getattr (e, '_sens', [])):
            n += 1; (s.entr, s.ord) = (id, n)
              # s.sens (the sense id number) is expected to be set by app code before here.
            if not s.sens: raise ValueError ("Sens.sens not set in sense #%d" % m)
            for m,x in enumerate (getattr (s, '_gloss', [])): (x.entr,x.sens,x.gloss) = (id, s.sens, m+1)
            for p,x in enumerate (getattr (s, '_pos',   [])): (x.entr, x.sens, x.ord) = (id, s.sens, p+1)
            for p,x in enumerate (getattr (s, '_misc',  [])): (x.entr, x.sens, x.ord) = (id, s.sens, p+1)
            for p,x in enumerate (getattr (s, '_fld',   [])): (x.entr, x.sens, x.ord) = (id, s.sens, p+1)
            for p,x in enumerate (getattr (s, '_dial',  [])): (x.entr, x.sens, x.ord) = (id, s.sens, p+1)
            for p,x in enumerate (getattr (s, '_lsrc',  [])): (x.entr, x.sens, x.ord) = (id, s.sens, p+1)
            for x in getattr (s, '_stagr', []): (x.entr, x.sens) = (id, s.sens)
            for x in getattr (s, '_stagk', []): (x.entr, x.sens) = (id, s.sens)
            for m,x in enumerate (getattr (s, '_xrslv', [])): (x.entr, x.sens, x.ord) = (id, s.sens, m+1)
            for m,x in enumerate (getattr (s, '_xref',  [])): (x.entr, x.sens, x.xref)= (id, s.sens, m+1)
            for x in getattr (s, '_xrer',  []): (x.xentr, x.xsens) = (id, s.sens)
